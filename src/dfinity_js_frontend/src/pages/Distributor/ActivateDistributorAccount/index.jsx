...
export default function ActivateDistributorAccount({ fetchDistributor }) {
  ...
  const [marketCoverage, setMarketCoverage] = useState("");
  const [regNo, setRegNo] = useState("");
  const [logo, setLogo] = useState("");

  let timerInSeconds = 0;

  const addDistributorInfo  = async () => {
    try {
      const distributor = {
        name,
        address,
      ...
      await createDistributorsCompany(distributor).then((res) => {
        fetchDistributor();
      });
        ...
        <div className="flex flex-row justify-center w-full max-w-[1083px]">
          <div className="flex flex-col items-end justify-start w-full">
            <Text size="12xl" as="p" className="mr-[473px]">
              Activate your Distributors account
            </Text>
                ...
                <div className="flex flex-row justify-start w-[92%] mt-4 ml-[3px] gap-6">
                  ...
                  <div className="flex flex-col items-start justify-center w-[49%] gap-0.5">
                    ...
                    <input
                      ...
                      placeholder="Market Coverage"
                      className="w-full bg-green-50 p-2 rounded-md border border-gray-500 text-black"
                      onChange={(e) => setMarketCoverage(e.target.value)}
                    />
                  </div>
                </div>
                <div className="flex flex-row justify-start w-[92%] mt-4 ml-[3px] gap-6">
